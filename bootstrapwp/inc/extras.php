...
function bootstrapwp_wp_title( $title, $sep ) {
	...
	if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
		$title .= " $sep " . sprintf( __( 'Page %s', 'bootstrapwp' ), max( $paged, $page ) );
	}
 ...
 * @return void
 */
function bootstrapwp_setup_author() {
	global $wp_query;
...
add_action( 'wp', 'bootstrapwp_setup_author' );
...
function modify_read_more_link() {
	...
	return '<p><a class="more-link btn btn-default" href="' . get_permalink() . '">En savoir plus...</a></p>';
}
